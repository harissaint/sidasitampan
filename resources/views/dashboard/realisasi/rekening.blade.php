                ...
                <div class="card-body">
                    <form action="" method="GET" class="row g-3 mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="tahapan_id" class="form-select" aria-label="Default select example" required>
                                ...
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item"><a
                                                href="{{ route('realisasi.rekening') }}?tahun={{request()->tahun}}&tahapan_id={{request()->tahapan_id}}&skpd={{ request()->skpd }}&level=1">Anggaran
                                                Rekening</a></li>
                                        @isset($breadcumb)
                                            @foreach ($breadcumb as $item)
                                                <li class="breadcrumb-item" aria-current="page">
                                                    <a
                                                        href="{{ route('realisasi.rekening') }}?tahun={{request()->tahun}}&tahapan_id={{request()->tahapan_id}}&skpd={{ request()->skpd }}&level={{ $item['level'] + 1 }}&code={{ $item['kode'] }}">{{ $item['nama'] }}</a>
                                                </li>
                                            @endforeach
                                        @endisset
                                    </ol>
                                </nav>
                                <div id="chart3" data-url="{{ route('realisasi.rekening') }}"></div>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('script')
    ...
    <script src="{{ asset('app/assets/js/index_realisasi_rekening.js') }}"></script>
