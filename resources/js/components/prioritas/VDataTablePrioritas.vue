...
import ModalAdd from './ModalAddPrioritas.vue';
import FormAdd from './FormAddPrioritas.vue';
...
const columns = [{
    ...
    render: function (data, type, row, meta) {
        ...
        return `
            <a href="${route('master.prioritas.show', row.id)}" class="btn btn-info btn-sm">
            ...
            <a href="${route('master.prioritas.report', row.id)}" class="btn btn-secondary btn-sm">
                <i class="bx bx-file"></i> Report Detail
            </a>

            <a href="${route('master.prioritas.recap', row.id)}" class="btn btn-secondary btn-sm">
...
export default {
    ...
    data() {
        return {
            ...
            options: {
                processing: true,
                serverSide: true,
                ajax: route('master.prioritas.index'),
            }
        }
    },
    // setup() {
    ...
    methods: {
        addData(formData) {
            // store data
            axios.post(route('master.prioritas.store'), formData)
                .then(response => {
        ...
        updateData(formData) {
            ...
            // update data
            axios.put(route('master.prioritas.update', formData.id), formData)
                .then(response => {
                    // close modal
        ...
        deleteData(formData) {
            // delete data
            axios.delete(route('master.prioritas.destroy', formData[0].value))
                .then(response => {
                    // close modal
