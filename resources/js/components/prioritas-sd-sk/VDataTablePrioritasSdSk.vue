...
import ModalAdd from './ModalAddPrioritasSdSk.vue';
import FormAdd from './FormAddPrioritasSdSk.vue';
...
const columns = [{
    ...
    render: function (data, type, row, meta) {
        ...
        return `
            <a href="${route('master.prioritas-sd-sk.show', row.id)}" class="btn btn-info btn-sm">
            ...
            <a href="${route('master.prioritas-sd-sk.report', row.id)}" class="btn btn-secondary btn-sm">
                <i class="bx bx-file"></i> Report Detail
            </a>

            <a href="${route('master.prioritas-sd-sk.recap', row.id)}" class="btn btn-secondary btn-sm">
...
export default {
    ...
    data() {
        return {
            ...
            options: {
                processing: true,
                serverSide: true,
                ajax: route('master.prioritas-sd-sk.index'),
            },
            tahapans: [],
        }
    },
    ...
    methods: {
        addData(formData) {
            // store data
            axios.post(route('master.prioritas-sd-sk.store'), formData)
                .then(response => {
        ...
        updateData(formData) {
            ...
            // update data
            axios.put(route('master.prioritas-sd-sk.update', formData.id), formData)
                .then(response => {
                    // close modal
        ...
        deleteData(formData) {
            // delete data
            axios.delete(route('master.prioritas-sd-sk.destroy', formData[0].value))
                .then(response => {
                    // close modal
