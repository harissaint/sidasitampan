...
import ModalAdd from './ModalAddSosmed.vue';
import FormAdd from './FormAddSosmed.vue';
                            ...
                            <form class="row g-3" onsubmit="return false;">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-select" id="category" name="category" required>
                                        ...
                                        <option value="SIDASI TAMPAN" ${row.category == 'SIDASI TAMPAN' ? 'selected' : ''}>SIDASI TAMPAN</option>
                                        <option value="SIPD PENGANGGARAN" ${row.category == 'SIPD PENGANGGARAN' ? 'selected' : ''}>SIPD PENGANGGARAN</option>
                                        <option value="SIPD-RI PENGANGGARAN" ${row.category == 'SIPD-RI PENGANGGARAN' ? 'selected' : ''}>SIPD-RI PENGANGGARAN</option>
                                        <option value="SIPD-RI PENATAUSAHAAN" ${row.category == 'SIPD-RI PENATAUSAHAAN' ? 'selected' : ''}>SIPD-RI PENATAUSAHAAN</option>
                                        <option value="Lain-lain" ${row.category == 'Lain-lain' ? 'selected' : ''}>Lain-lain</option>
                                    </select>
                                </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content border-top border-0 border-4 border-danger">
                        ...
                        <div class="modal-body">
                            <form class="row g-3" onsubmit="return false;">
                                <input type="hidden" name="id" value="${row.id}">
                                <p>Apakah anda yakin ingin menghapus link sosmed: <strong>${row.link}</strong>?</p>
                                <!-- Add more form fields here -->
                                <div class="d-flex justify-content-end">
...
export default {
    ...
    data() {
        return {
            ...
            options: {
                processing: true,
                serverSide: true,
                ajax: route('setting.sosmed.index'),
            }
        }
    },
    // setup() {
    ...
    methods: {
        addData(formData) {
            // store data
            axios.post(route('setting.sosmed.store'), formData)
                .then(response => {
        ...
        updateData(formData) {
            ...
            // update data
            axios.put(route('setting.sosmed.update', formData.id), formData)
                .then(response => {
                    // close modal
        ...
        deleteData(formData) {
            // delete data
            axios.delete(route('setting.sosmed.destroy', formData[0].value))
                .then(response => {
                    // close modal
