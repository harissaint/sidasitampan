...
const columns = [{
    ...
    title: 'Rekening',
    render: function (data, type, row, meta) {
        return row.sipd.kode_akun + ' - ' + row.sipd.nama_akun;
    },
}, {
...
export default {
    ...
    data() {
        return {
            ...
            options: {
                ...
                serverSide: false,
                ajax: {
                    url: route('master.prioritas-sd.report', this.prioritas_id),
                    type: 'GET',
                    data: {
    ...
    mounted() {
        ...
        $('#dt-table').on('change', '.nilai', function () {
            ...
            axios.post(route('master.prioritas-sd.updatenilai', data))
                .then(response => {
