...
import ModalAdd from './ModalAddPrioritasSd.vue';
import FormAdd from './FormAddPrioritasSd.vue';
...
const columns = [{
    ...
    title: 'Jumlah Maping',
    render: function (data, type, row, meta) {
        return row.maps.length + ' Rekening';
    },
}, {
    data: 'id',
    title: 'Aksi',
    render: function (data, type, row, meta) {
        ...
        let tahapans = '';
        row.tahapans.forEach(function (tahapan) {            
            tahapans += `<option value="${tahapan.id}" ${tahapan.id == row.tahapan.id ? 'selected' : ''}>${tahapan.nama}</option>`;
        });

        return `
            <a href="${route('master.prioritas-sd.show', row.id)}" class="btn btn-info btn-sm">
                        ...
                        <div class="modal-body">
                            <form class="row g-3" onsubmit="return false;">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-select" id="tahapan_id" name="tahapan_id" required>
                                        ${tahapans}
                                    </select>
                                </div>
            ...
            <a href="${route('master.prioritas-sd.report', row.id)}" class="btn btn-secondary btn-sm">
                <i class="bx bx-file"></i> Report Detail
            </a>

            <a href="${route('master.prioritas-sd.recap', row.id)}" class="btn btn-secondary btn-sm">
...
export default {
    ...
    data() {
        return {
            ...
            options: {
                processing: true,
                serverSide: true,
                ajax: route('master.prioritas-sd.index'),
            },
            tahapans: [],
        }
    },
    ...
    methods: {
        addData(formData) {
            // store data
            axios.post(route('master.prioritas-sd.store'), formData)
                .then(response => {
        ...
        updateData(formData) {
            ...
            formData = {
                ...
                tahapan_id: formData[2].value,
            };
            // update data
            axios.put(route('master.prioritas-sd.update', formData.id), formData)
                .then(response => {
                    // close modal
        ...
        deleteData(formData) {
            // delete data
            axios.delete(route('master.prioritas-sd.destroy', formData[0].value))
                .then(response => {
                    // close modal
